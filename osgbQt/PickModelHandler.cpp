...
void PickModelHandler::createBackboard(Image* image, Image* image2, float width, float height, float deep, float eyeDis)
{
	...
	direction.normalize();
	up.normalize();
	//cout << "center:" << center << "," << "eye:" << eye << ",direction" << direction << endl;
	...
	//v2->push_back(point3);
	//v2->push_back(point4);
	v2->push_back(point1 - left * tEyeDis);
	v2->push_back(point2 - left * tEyeDis);
	v2->push_back(point3 - left * tEyeDis);
	v2->push_back(point4 - left * tEyeDis);
...
bool PickModelHandler::handle(const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &aa)
{
		...
		(ea.getModKeyMask() & GUIEventAdapter::MODKEY_CTRL)*/)
	{
		...
		int res = pickAPI(clickX, clickY, 1);
	}
		...
		if (collisionFlag) 
		{
			...
			for (int k = 1; k < 20; k++)
			{
				...
				for (int i = 0; i < 2; i++){
					...
					Vec3d tmp2 = initVec + tmp;
					if (tmp2.x() < _roomWidthX  && tmp2.y()  < _roomHeightY && tmp2.z() < _roomWidthZ
						&& tmp2.x() > -_roomWidthX  && tmp2.y()  > -_roomHeightY && tmp2.z() > -_roomWidthZ) {
...
MatrixTransform* PickModelHandler::duplicateOneObj(MatrixTransform * matrixTrans)
{
	...
	ref_ptr<MatrixTransform> trans = new MatrixTransform(*matrixTrans, CopyOp::SHALLOW_COPY);
...
void PickModelHandler::addGround(float widthX, float widthZ, float heightY)
{
	if (_hasGround){
		...
		_root->removeChild(_roomTrans);
		_collisionWorld->removeCollisionObject(_btground);
		delete _cs;
		//return;
	}
	...
	_roomTrans = new osg::MatrixTransform;
	...
	_collisionWorld->addCollisionObject(_btground);
...
MatrixTransform* PickModelHandler::createBackBox(Vec3f center)
{
	//ref_ptr<ShapeDrawable> boxDrawable = new ShapeDrawable(new Box(Vec3(0.,0.,0.), 1.f));
	ref_ptr<ShapeDrawable> boxDrawable = new ShapeDrawable(new Box(center, 1.f));
...
bool PickModelHandler::scaleOneObj(MatrixTransform* model, MatrixTransform* box, btCollisionObject* collisionObj, Vec3d scaleVec,bool isDetectCollision)
{
	...
	if (scaleVec.x() < 1.0)
	{
		...
		if (bb.radius() < MODELSIZEMIN)
		{
			cout << "obj too small! can't scale smaller!" << endl;
			return false;
		}
	}
...
void PickModelHandler::handleKeyEvent(const osgGA::GUIEventAdapter &ea, osgGA::GUIActionAdapter &aa)
{
...

	{
		...
		if (isPopHistory) {
			...
			scaleRoom(scaleFactor.x(), scaleFactor.y(), scaleFactor.z());
		}
...
{
	_roomWidthX = _roomWidthX * scaleX;
	_roomWidthZ = _roomWidthZ * scaleY;
	_roomHeightY = _roomHeightY * scaleZ;
	cout << "add ground" << endl;
	addGround(_roomWidthX, _roomWidthZ, _roomHeightY);
